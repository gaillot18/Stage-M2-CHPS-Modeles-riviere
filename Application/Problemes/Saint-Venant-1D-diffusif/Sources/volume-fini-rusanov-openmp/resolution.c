...
void calculer_Delta_t_n(
    ...
    # pragma omp parallel for schedule(static) reduction(min:Delta_t_n)
    for (int k = 1 ; k < K + 1 ; k ++){
...
void calculer_lambda_star_n(
    ...
    # pragma omp parallel for private(lambda_star_gauche, lambda_star_droite) schedule(static)
    for (int k = 0 ; k < K + 1 ; k ++){
...
void appliquer_schema_Q(
    ...
    # pragma omp parallel for private(d_sur_ds_A_n_plus_1_k, Q_1_n_plus_1_k, Q_2_n_plus_1_k, sgn_Q_2_n_plus_1_k) schedule(static)
    for (int k = 1 ; k < K + 1 ; k ++){
        ...
        Q_1_n_plus_1_k = (
            ...
            (
                b[k] * 
                pow(r[k], 2) * pow((b[k] + 2.0 * A[idx(k, n + 1)] / b[k]), 4.0 / 3.0)
            )
        );

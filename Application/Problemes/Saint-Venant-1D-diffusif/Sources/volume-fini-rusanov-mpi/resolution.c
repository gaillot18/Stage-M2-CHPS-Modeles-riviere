...
void calculer_lambda_star_n(
    ...
    for (int k = k_boucle_debut - 1 ; k < k_boucle_fin ; k ++){
    ...
    for (int k = k_boucle_debut ; k < k_boucle_fin ; k ++){
        ...
        A_divise[idx(k, n + 1)] = (
            -0.5 * (Delta_t[n] / Delta_s_divise[k]) * (
                Q_divise[idx(k + 1, n)] -
                Q_divise[idx(k - 1, n)] -
                lambda_star_n[k] * (A_divise[idx(k + 1, n)] - A_divise[idx(k, n)]) +
                lambda_star_n[k - 1] * (A_divise[idx(k, n)] - A_divise[idx(k - 1, n)])
            ) +
            A_divise[idx(k, n)] +
...
void appliquer_schema_Q(
    ...
    for (int k = k_boucle_debut ; k < k_boucle_fin ; k ++){
        ...
        if (k == k_boucle_debut && voisin[0] == -1){
            d_sur_ds_A_n_plus_1_k = (
                (A_divise[idx(k + 1, n + 1)] - A_divise[idx(k, n + 1)]) /
                (0.5 * (Delta_s_divise[k] + Delta_s_divise[k + 1]))
            );
        }
        else if (k == k_boucle_fin - 1 && voisin[1] == -1){
            d_sur_ds_A_n_plus_1_k = (
                (A_divise[idx(k, n + 1)] - A_divise[idx(k - 1, n + 1)]) /
                (0.5 * (Delta_s_divise[k - 1] + Delta_s_divise[k]))
            );
        }
        else{
            d_sur_ds_A_n_plus_1_k = (
                (A_divise[idx(k + 1, n + 1)] - A_divise[idx(k - 1, n + 1)]) /
                (0.5 * Delta_s_divise[k - 1] + Delta_s_divise[k] + 0.5 * Delta_s_divise[k + 1])
            );
        }


        Q_1_n_plus_1_k = (
            pow(A_divise[idx(k, n + 1)], 10.0 / 3.0) /
            (
                b_divise[k] * 
                pow(r_divise[k], 2) * pow((b_divise[k] + 2.0 * A_divise[idx(k, n + 1)] / b_divise[k]), 4.0 / 3.0)
            )
        );
        ...
        Q_2_n_plus_1_k = (
            -d_sur_ds_A_n_plus_1_k +
            0.5 * (A_divise[idx(k, n + 1)] / b_divise[k]) * b_prime_divise[k] +
            b_divise[k] * (
                S_0_divise[k] + 
                (q_divise[idx(k, n + 1)] * v_divise[idx(k, n + 1)]) 
                / (g * A_divise[idx(k, n + 1)])
            )
        );
...
void resoudre_volume_fini_rusanov(
    ...
    lambda_star_n = (double *)malloc((K_divise + 2) * sizeof(double));

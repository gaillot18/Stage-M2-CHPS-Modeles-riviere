...
void appliquer_schema_Q(
    ...
    for (int k = 1 ; k < K + 1 ; k ++){
        ...
        Q[idx(k, n + 1)] = (
            -0.5 * (Delta_t[n] / Delta_s[k]) * (
                pow(Q[idx(k + 1, n)], 2) / A[idx(k + 1, n)] -
                pow(Q[idx(k - 1, n)], 2) / A[idx(k - 1, n)] +
                0.5 * g * (
                    pow(A[idx(k + 1, n)], 2) / b[k + 1] -
                    pow(A[idx(k - 1, n)], 2) / b[k - 1]
                ) -
                lambda_star_n[k] * (Q[idx(k + 1, n)] - Q[idx(k, n)]) +
                lambda_star_n[k - 1] * (Q[idx(k, n)] - Q[idx(k - 1, n)])
            ) +
            Q[idx(k, n)] +
            Delta_t[n] * (
                g * 
                (S_0[k] - S_f_n[k]) *
                A[idx(k, n)] + q[idx(k, n)] * v[idx(k, n)]
            )
        );
